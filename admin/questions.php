...
<div class="mb-8">
    <nav class="flex items-center space-x-2 text-slate-400 text-xs font-semibold mb-2">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
            ...
            <p class="text-slate-500 text-xs font-semibold">Manage and organize the questions for this quiz.</p>
        </div>
        <div class="flex-shrink-0">
            <a href="add-question.php?quiz_id=<?php echo $quiz_id; ?>" class="bg-primary-600 hover:bg-primary-700 text-white font-bold px-6 py-3 rounded-full shadow-md text-xs transition-all focus:outline-none hover:scale-105 flex items-center">
                <i class="fas fa-plus mr-2 text-[10px]"></i>Add Question
            </a>
        </div>
    </div>
</div>
...
<div class="space-y-6">
            ...
            <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm relative overflow-hidden">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start gap-4 mb-4">
                    <div class="flex items-start min-w-0">
                        <div class="w-10 h-10 rounded-full bg-primary-50 border border-primary-100/30 flex items-center justify-center font-bold text-primary-600 mr-3 text-xs select-none flex-shrink-0">
                            Q<?php echo $index + 1; ?>
                        </div>
                        <div class="min-w-0">
                            <h5 class="font-extrabold text-slate-900 text-base mb-2 leading-normal"><?php echo sanitize($q['question_text']); ?></h5>
                            <div class="flex flex-wrap gap-2">
                                <span class="bg-slate-100 text-slate-550 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider">
                                    Marks: <?php echo $q['marks']; ?>
                                </span>
                                <span class="bg-primary-50 text-primary-600 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider text-capitalize">
                ...
                <div class="bg-slate-50 border border-slate-100 p-4 rounded-2xl md:ml-13">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <?php foreach ($options as $opt): ?>
                            <div class="flex items-center p-3 rounded-xl text-xs font-semibold <?php echo $opt['is_correct'] ? 'bg-emerald-50/50 border border-emerald-100/50 text-emerald-800' : 'bg-transparent text-slate-500'; ?>">
    ...
    <?php else: ?>
        <div class="bg-white border border-slate-200 p-10 rounded-3xl text-center shadow-sm max-w-md mx-auto my-8">
            ...
            <p class="text-slate-400 text-xs leading-relaxed max-w-xs mx-auto">This quiz is empty. Start adding questions to build it.</p>
