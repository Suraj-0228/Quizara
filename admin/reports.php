...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
    <div>
        ...
        <p class="text-slate-500 text-sm mb-0">Analytics and performance overview.</p>
    </div>
    <div class="flex-shrink-0 print:hidden">
        <button onclick="window.print()" class="border border-primary-200 text-primary-600 hover:bg-primary-50 font-bold px-5 py-2.5 rounded-full shadow-sm text-sm transition-all flex items-center focus:outline-none">
            <i class="fas fa-print mr-2 text-xs"></i>Print Report
        </button>
    </div>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 print:hidden">
    <div class="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm text-center">
        <div class="text-slate-400 text-xs font-bold uppercase tracking-wider mb-2">Total Attempts</div>
        <div class="text-4xl font-extrabold text-slate-800"><?php echo $total_attempts; ?></div>
    </div>
    <div class="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm text-center">
        <div class="text-slate-400 text-xs font-bold uppercase tracking-wider mb-2">Average Score</div>
        <div class="text-4xl font-extrabold text-sky-500"><?php echo $avg_score; ?>%</div>
    </div>
    <div class="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm text-center">
        <div class="text-slate-400 text-xs font-bold uppercase tracking-wider mb-2">Pass Rate</div>
        ...
        <table class="w-full text-left text-sm text-slate-600 border-collapse">
            <thead class="bg-slate-50 border-b border-slate-100">
                <tr>
                    ...
                    <th class="px-6 py-3.5 text-sm font-bold text-slate-400 uppercase tracking-wider text-center">Status</th>
                    <th class="px-6 py-3.5 text-sm font-bold text-slate-400 uppercase tracking-wider text-right">Date</th>
                </tr>
            </thead>
            <tbody>
                <?php if (count($attempts) > 0): ?>
                    <?php foreach ($attempts as $attempt): ?>
                        ...
                        <tr class="border-b border-slate-100 last:border-0 hover:bg-slate-50/30 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right text-slate-400 text-sm font-semibold">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="5" class="text-center py-12">
                            ...
                            <p class="text-slate-400 text-sm">Students haven't completed any quizzes yet.</p>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
