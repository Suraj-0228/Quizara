    ...
    <div class="relative z-1 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
            <span class="inline-flex items-center bg-white/20 text-white border-0 px-3.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider mb-3">
                <i class="fas fa-search mr-2 text-[10px]"></i> Quiz Library
            </span>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="quizGrid">
        ...
        <?php foreach ($quizzes as $quiz): ?>
            <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm hover:shadow-premium hover:-translate-y-1.5 transition-all duration-300 flex flex-col h-full quiz-item" data-category="<?php echo sanitize($quiz['category_name']); ?>">
                <div class="flex justify-between items-start mb-4">
                    <div class="bg-primary-50 border border-primary-100/30 text-primary-600 text-[10px] font-bold px-3 py-1 rounded-full"><?php echo sanitize($quiz['category_name']); ?></div>
                    <div class="text-xs font-bold <?php echo ($quiz['time_limit'] > 0) ? 'text-amber-500' : 'text-emerald-500'; ?> flex items-center">
                ...
                <p class="text-slate-500 text-xs leading-relaxed mb-6 flex-grow line-clamp-3"><?php echo sanitize($quiz['description']); ?></p>

                <div class="flex items-center text-slate-400 text-xs mb-4 font-semibold">
                ...
                <?php if ($quiz['question_count'] > 0): ?>
                    <div class="flex space-x-1.5 bg-slate-50 p-1 rounded-2xl mt-auto">
                        ...
                        <?php if (in_array($quiz['highest_mode_completed'], ['low', 'medium', 'high'])): ?>
                            <button class="flex-1 text-center py-2 text-xs font-bold bg-primary-100 text-primary-600 rounded-xl flex items-center justify-center space-x-1 border border-primary-200/30" disabled title="Completed">
                                <i class="fas fa-check-circle text-[10px]"></i>
                                <span>Low</span>
                            </button>
                        <?php else: ?>
                            <a href="take-quiz.php?id=<?php echo $quiz['id']; ?>&mode=low" class="flex-1 text-center py-2 text-xs font-bold text-primary-600 hover:bg-primary-100 hover:text-primary-700 rounded-xl flex items-center justify-center space-x-1 transition-all" title="Start Low Mode">
                        ...
                        <?php elseif (in_array($quiz['highest_mode_completed'], ['low'])): ?>
                            <a href="take-quiz.php?id=<?php echo $quiz['id']; ?>&mode=medium" class="flex-1 text-center py-2 text-xs font-bold text-primary-600 hover:bg-primary-100 hover:text-primary-700 rounded-xl flex items-center justify-center space-x-1 transition-all" title="Start Medium Mode">
                                <i class="fas fa-play text-[9px]"></i>
                                <span>Med</span>
                            </a>
                        <?php else: ?>
                            <button class="flex-1 text-center py-2 text-xs font-bold text-slate-400 bg-slate-100/50 rounded-xl flex items-center justify-center space-x-1 cursor-not-allowed" disabled title="Locked">
                        ...
                        <?php elseif (in_array($quiz['highest_mode_completed'], ['medium'])): ?>
                            <?php if ($quiz['is_purchased']): ?>
                                <a href="take-quiz.php?id=<?php echo $quiz['id']; ?>&mode=high" class="flex-1 text-center py-2 text-xs font-bold text-primary-600 hover:bg-primary-100 hover:text-primary-700 rounded-xl flex items-center justify-center space-x-1 transition-all" title="Start High Mode">
                                    <i class="fas fa-play text-[9px]"></i>
                                    <span>High</span>
                                </a>
                            <?php else: ?>
                                <a href="upgrade-premium.php?quiz_id=<?php echo $quiz['id']; ?>" class="flex-1 text-center py-2 text-xs font-bold text-amber-600 bg-amber-50 hover:bg-amber-100 rounded-xl flex items-center justify-center space-x-1 transition-all border border-amber-200/30" title="Upgrade to Premium">
                ...
                <?php else: ?>
                    <div class="bg-slate-50 text-center py-2 rounded-xl text-slate-400 font-bold text-xs mt-auto">
    ...
    <?php else: ?>
        <div class="col-span-full text-center py-12 bg-white rounded-3xl border border-slate-200 shadow-sm p-10">
            ...
            <a href="quizzes.php" class="inline-block border border-primary-200 text-primary-600 hover:bg-primary-50 font-bold px-6 py-2 rounded-full text-xs mt-4 transition-all">Clear Filters</a>
        </div>
    <?php endif; ?>
</div>
...
<?php if (isset($total_pages) && $total_pages > 1): ?>
    <div class="flex justify-center mt-10">
        <nav class="flex items-center space-x-1 bg-white border border-slate-200 rounded-full p-1.5 shadow-sm">
            ...
            <!-- Previous Button -->
            <a class="px-4 py-2 rounded-full text-xs font-semibold text-slate-500 hover:bg-slate-50 hover:text-primary-600 transition-all <?php echo ($page <= 1) ? 'pointer-events-none opacity-50' : ''; ?>" href="?page=<?php echo $page - 1; ?><?php echo $queryString; ?>">
                <i class="fas fa-chevron-left mr-1.5 text-[10px]"></i> Prev
            </a>

            <!-- Page Numbers -->
            <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                <a class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all <?php echo ($page == $i) ? 'bg-primary-600 text-white shadow-sm' : 'text-slate-655 hover:bg-slate-50 hover:text-primary-600'; ?>" href="?page=<?php echo $i; ?><?php echo $queryString; ?>">
                    <?php echo $i; ?>
                </a>
            <?php endfor; ?>

            <!-- Next Button -->
            <a class="px-4 py-2 rounded-full text-xs font-semibold text-slate-500 hover:bg-slate-50 hover:text-primary-600 transition-all <?php echo ($page >= $total_pages) ? 'pointer-events-none opacity-50' : ''; ?>" href="?page=<?php echo $page + 1; ?><?php echo $queryString; ?>">
                Next <i class="fas fa-chevron-right ml-1.5 text-[10px]"></i>
            </a>
        </nav>
    </div>
<?php endif; ?>
